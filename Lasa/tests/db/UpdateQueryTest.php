...
class UpdateQueryTest extends \PHPUnit\Framework\TestCase {
	...
	function パターン１_Objectを引数に一個持ったUpdateメソッド() {
		...
		$this->assertStringContainsString("hoge_value=:hoge_value", $query);
		$this->assertStringContainsString("id=:id", $where);
		$this->assertStringContainsString("user_id=:user_id", $query);
		$this->assertStringContainsString("hoge_type=:hoge_type", $query);
		$this->assertStringContainsString("hoge_value=:hoge_value", $query);
	...
	function パターン２_Objectを引数に一個持ったUpdateメソッド（where句で条件を指定）() {
		...
		$this->assertStringNotContainsString("id=:id", $where);
		$this->assertStringContainsString("user_id=:userId", $where);
		$this->assertStringContainsString("hoge_type=:type", $where);
	...
	function パターン４_2_パラメーターを引数に持ったUpdateメソッドでwhere句指定() {
		...
		$this->assertStringContainsString("hoge_type=:hoge_type", $query);
		$this->assertStringContainsString("user_id=:hogehoge", $where);
		$this->assertStringNotContainsString("id=:id", $where);
	...
	function パターン5_エラーになるパターン１() {
		...
		try {
			...
			$this->assertStringContainsString("updateInvalidPattern", $e->getMessage());
		}
	}
