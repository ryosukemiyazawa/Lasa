 ...
 * Viewの拡張が出来るようにする
 */
class ViewExtendsTest extends PHPUnit_Framework_TestCase {
	...
	function シンプルな拡張のテストする(){
		...
		$this->assertContains("<p>this is base</p>", $content);
		...
		$content = $view->getContent();
		$this->assertContains("<div>this is extends</div>", $content);
		...
		$content = $view->getContent();
		$this->assertContains("<div>アノテーションでextendsした場合</div>", $content);
	...
	function 拡張して機能追加する(){
		...
		$content = $view->getContent();
		$this->assertContains("<div>アノテーションでextendsした場合</div>", $content);
		$this->assertContains("<div>機能追加です</div>", $content);
