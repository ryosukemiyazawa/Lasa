	...
	function __toString() {
		...
		switch ($this->prefix) {
			...
			case "insert":
				...
				if ($this->where && strlen($this->where)) {
			...
			case "select":
				...
				if ($this->group && strlen($this->group)) {
					$sql .= " group by " . $this->group;
				}
				if ($this->having && strlen($this->having)) {
					$sql .= " having " . $this->having;
				}
				if ($this->order && strlen($this->order)) {
...
class InsertQueryFactory extends QueryFactory {
	...
	function finalize() {
		...
		$sql = "(" . implode(",", $columns) . ")";
		$sql .= " values(" . implode(",", array_map(function ($a) {
			return ":" . $a;
		}, $columns)) . ")";
		$this->sql = $sql;
	}
